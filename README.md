...
**Developed by: Shayon Boidya**

---

## Features

- **Automatic USB detection** – polls for newly inserted removable drives (Windows) using `GetDriveTypeW` API
...
- **Robust logging** – all actions are logged to a file with automatic fallback paths
- **Writable paths by default** – uses relative paths to avoid Windows permission issues
...
- Packages listed in `requirements.txt`:
  ...
  - `pywin32` – Windows API bindings (PID check, drive detection)
...
|-----|-------------|---------|
| `backup_dir` | Where backed‑up files are stored (relative to project dir) | `./backups` |
...
| `log_file` | Path to the daemon log file | `./usb_backup.log` |
...
1. The daemon checks for an already‑inserted USB drive using Windows `GetDriveTypeW` API.
...
├── usb_device.py         # Windows removable‑drive detection (GetDriveTypeW)
...
1. **USB detection** – `usb_device.list_removable_drives()` uses `GetDriveTypeW` to detect only `DRIVE_REMOVABLE` volumes (actual USB sticks).
...
- The daemon currently backs up from **one USB drive at a time** (the first detected removable drive).
- Only drives detected as `DRIVE_REMOVABLE` (type 2) are considered — this excludes fixed disks and network drives.
- Logs are written to the file configured in `config.json`; if a console is attached, they also appear on stdout.
- The system automatically falls back to alternative log paths if the primary path is not writable.

## License

This project is developed and maintained by **Shayon Boidya**. Feel free to fork, modify, and contribute!
